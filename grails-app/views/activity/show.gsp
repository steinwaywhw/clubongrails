	...
	<body>
		...
		<div id="show-activity" class="content scaffold-show" role="main">
			...
			<ol class="property-list activity">
				<g:render template="showBasic"/>
				<g:render template="showReport"/>


				<g:if test="${activityInstance?.logs}">
				<li class="fieldcontain">
					<span id="logs-label" class="property-label">
						<g:message code="activity.logs.label" default="Logs"/>
					</span>
					
					<g:each in="${activityInstance.logs}" var="d">
					<span class="property-value" aria-labelledby="logs-label">
						<g:link controller="activityLog" action="show" id="${d.id}">${d?.encodeAsHTML()}</g:link>
					</span>
					</g:each>
